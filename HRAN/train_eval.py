...
class RunModel:
    ...
    def __init__(self, data: Data, modelname, optimizer_method="Adam", learning_rate=0.001, ent_vec_dim=200,
                 rel_vec_dim=200, num_iterations=100, batch_size=128, decay_rate=0., input_dropout=0.,
        ...
        self.adjacencies = data.get_adjacencies(alpha)
    ...
    def evaluate(self, model, eval_data, iteration):
        ...
        for batch_data, batch_num in self.data.get_batch_eval_data(self.batch_size, eval_data):
            ...
            rela_id = batch_data[:, 1].to(device)
            tail_id = batch_data[:, 2].to(device)

            embedded_ent, embedded_rel = model.hgn(self.adjacencies)
    ...
    def train_and_eval(self):
        ...
        elif self.model_name.lower() == "hran":
            model = HRAN(self.data, self.ent_vec_dim, self.rel_vec_dim, **self.kwargs)
